...
int GetPvLine(const int depth, S_BOARD *pos) {
	...
	while (move != NOMOVE && count < depth) {
		...
		if (MoveExists(pos, move)) {
			MakeMove(pos, move);
			pos->PvArray[count++] = move; // Add moves to PvArray
		}
		else {
			break;
		}
...
void InitHashTable(S_HASHTABLE *table) {
	table->numEntries = PvSize / sizeof(S_HASHENTRY); // Set table size
	table->numEntries -= 2; // -2 for safety
	if (table->pTable != NULL)
		free(table->pTable); // Free any memory
	table->pTable = (S_HASHENTRY *)malloc(table->numEntries * sizeof(S_HASHENTRY)); // memory allocate enough memory
